...
import { Calendar, X, Send, ArrowLeft, Clock, Tag, User, MessageCircle, Trash2 } from 'lucide-react';
import SEO from '../components/SEO';

export default function SeputarHimmah() {
  const { berita, komentar, saveKomentarBaru, isLoggedIn } = useApp();
  const [selectedBerita, setSelectedBerita] = useState(null);
  const [teksKomentar, setTeksKomentar] = useState('');

  const handleKirimKomentar = (beritaId) => {
    if (!teksKomentar.trim()) return;
    saveKomentarBaru(beritaId, {
      teks: teksKomentar,
  ...
  const handleDeleteKomentar = (beritaId, index) => {
    const komentarBerita = komentar[beritaId] || [];
    const updated = komentarBerita.filter((_, i) => i !== index);
    saveKomentarBaru(beritaId, updated, true);
  };

  // Tampilan detail berita
  if (selectedBerita) {
    const komentarBerita = komentar[selectedBerita.id] || [];

    return (
      <>
        <SEO title={selectedBerita.judul} description={selectedBerita.kontenHTML?.replace(/<[^>]*>/g, '').substring(0, 150)} />
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10 pb-12">
            ...
            <div className="glass rounded-2xl p-6 sm:p-8 mb-8">
              ...
              <div className="flex items-center justify-between mt-6">
                <ShareButtons
                  ...
                  text={selectedBerita.kontenHTML?.replace(/<[^>]*>/g, '').substring(0, 200) || ''}
                />
              </div>
            </div>
            ...
            <div className="glass rounded-2xl p-6 sm:p-8 space-y-4">
              ...
              {komentarBerita.map((k, i) => (
                <div key={i} className="bg-white/5 rounded-lg p-4 relative">
                  ...
                  {isLoggedIn && (
                    <button
                      onClick={() => handleDeleteKomentar(selectedBerita.id, i)}
  ...
  return (
    <>
      <SEO title="Berita" description="Berita dan kegiatan terkini HIMMAH NW Komisariat STMIK." />
      <div className="min-h-screen pb-16">
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-3 text-green-400/60 text-xs flex items-center justify-between">
                      ...
                      <span className="flex items-center gap-1">
                        <MessageCircle size={12} /> {komentar[item.id]?.length || 0}
                      </span>
                    </div>
                  </div>
                </article>
